...
file_name = paste0("graph_FI_em_market_indices_zoom", ".pdf" )
...
getDataDI("RUSN1PS.D", "CHNN1PS.D", "BRAN1PS.D", "INBSEX.D","ARGENBCX.D")
# pRev::getDataDI("M186SPASTT01XO.M")
...
data = data %>%
  ...
  mutate(type = case_when(label == "Argentine" ~ "Pays émergents", TRUE ~ "Pays émergents"))
...
graph_fi_em_market_indices = ggplot(data = data, aes(x = time , y = value, colour = label )) +
  geom_line(size = 1.2) +
...
export_minio_graph(graph_fi_em_market_indices, perim = "FI", folder_name = "fi_em_market_indices")
