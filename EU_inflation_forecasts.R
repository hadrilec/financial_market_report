...
myUrl <- paste0(path_ECB_FM,sprintf("Q.U2..POINT+SUM.%s+%s+LT.Q.", yearc, yearc+1))
...
data = dt %>% 
  ...
  filter(year_date %in% c(yearc, yearc-1, yearc-2)) %>% 
  filter(UNIT == "PCPA") %>% 
...
data_point2 = data_point %>% filter(FCT_HORIZON == 2021)

data_line2 = data_line %>% filter(FCT_HORIZON == 2021)

ggplot() +
  geom_point(data=data_point2, aes(x = obsTime, y = obsValue), group=1) +
  geom_line(data=data_line2, aes(x = obsTime, y = obsValue), colour = "red", group=1) +
  geom_point(data=data_line2, aes(x = obsTime, y = obsValue), colour = "red", group=1) +
  facet_wrap(~var_horizon) +
  ggthemes::theme_stata() +
  ggtitle("Prévisions d'inflation en zone euro - enquête au près des prévisionnistes") +
  labs(subtitle = "Source: BCE", caption = "") +
