...
file_name = paste0("graph_FI_em_market_indices", ".pdf" )
...
for(ivar in 1:length(list_var_pRev)){
  ...
  df[,"value"] = df[,"value"] * 100 / df[min(which(df[,"year"] == 2015 & df[,"month"] == 1)),"value"]
...
graph_fi_em_market_indices = ggplot(data = data, aes(x = data_time , y = value, colour = label )) +
  geom_line(size = 1) +
  ggtitle("Indices boursiers des pays émergents (base 100 janvier 2015)") +
  labs(subtitle = graph_subtitle) +
  scale_y_continuous(breaks = yaxis_breaks, sec.axis = sec_axis(~ .* (400/250)) ) +
