...
data = data_10y %>%
  filter(label %in%  c("France", "Allemagne", "Etats-Unis", "Royaume-Uni", "Espagne", "Italie")) %>%
...
xaxis_breaks = seq.Date(from = min(data_time), to = max(data_time), by = "6 month")
...
graph_WORLD_gov_bond = ggplot(data = data,
  ...
  geom_line(size = 1) +
  ggtitle("Taux souverains américains et européens à 10 ans") +
...
export_minio_graph(graph_WORLD_gov_bond, perim = "FI", folder_name = "WORLD_gov_bond", update = TRUE)
